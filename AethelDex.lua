	...
	function ScrollBar.new(hor)
		...
		button1.InputBegan:Connect(function(input)
			...
			while buttonPress do
				if os.clock() - buttonTick >= 0.3 and newMt:CanScrollUp() then
					newMt:ScrollUp()
				end
				task.wait()
			end
		...
		button2.InputBegan:Connect(function(input)
			...
			while buttonPress do
				if os.clock() - buttonTick >= 0.3 and newMt:CanScrollDown() then
					newMt:ScrollDown()
				end
				task.wait()
			end
		...
		scrollThumbFrame.InputBegan:Connect(function(input)
			...
			while thumbFramePress and not f.checkMouseInGui(scrollThumb) do
				if os.clock() - thumbFrameTick >= 0.3 then
					doTick()
				end
				task.wait()
			end
...
function f.refreshExplorer()
	...
	--updateDebounce = true
	--Services.RunService.RenderStepped:task.wait()
...
function f.addObject(obj,noupdate,recurse)
	pcall(function()
		...
		local newNode = {
			...
			Depth = f.depth(obj),
			UID = os.clock()--RMD[v.ClassName] and (RMD[v.ClassName].ExplorerOrder or 999) or 999
		}
